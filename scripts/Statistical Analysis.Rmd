...
```{r}
g <- ggplot(crimes, aes(premise, decreasing = F))
...
crimes$premise <- forcats::fct_lump(crimes$premise, n = 6)
fct_reorder(premise, premisecount)
premisecount <- count(premise)
...
# make table to order for ordered bar plot
      ...
      bar_list<- as.list(barplot(premise_table[ordered_premise]))
      
# want to selec top 10 crime types
premise_table <- table(crimes$premise)
ordered_premise <- order(premise_table, decreasing = TRUE)
