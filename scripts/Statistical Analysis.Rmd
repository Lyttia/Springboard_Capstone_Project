...
1. Counts:
We have kept a total of 159,319 crime observations that have been reported to Phoenix Police Department within Phoenix zipcodes between November 1st, 2015 to June 25th, 2018.

How many crimes occur each month in 2016 and 2017?
...
How many crimes are reported for each zipcode?
...
g + geom_bar(aes(fill=category), width = 0.5) + 
  theme(axis.text.x= element_text(angle = 65, vjust= 0.6)) + 
  labs(title= "2017 Zipcode Frequency Bar Chart")
#ggsave("2017 Zipcode Frequency Bar Chart.png")
```


Which zipcodes are high crime count and which are low?
...
How many crimes occur in the highest crime count zip each month in 2016 and 2017?
...
Limitations: 
Accounts only for crimes reported to Phoenix PD. There may be many other crimes reported to surrounding PDs.
